...
# Depth metrics (RMSE, AbsRel, Acc[<1.25], etc.)
from utils.metrics import compute_depth_metrics

# Model definitions:
from models.dust3r import DUSt3R   # DUSt3R model (assumed adapted for monocular input)
from mast3r.model import AsymmetricMASt3R  # MASt3R model class
from dust3r.inference import inference    # inference helper for MASt3R

# Evaluation dataset (for monocular depth estimation)
from data.freiburg_evaluate_dataset import FreiburgEvaluateThermalDataset

def parse_args():
    parser = argparse.ArgumentParser(description="Compare fine-tuned DUSt3R vs. combined (base DUSt3R + MASt3R) for monocular depth estimation on thermal data")
...
def main(args):
    ...
    # Get normalization parameters from config (or defaults)
    mean = config.get("data", {}).get("mean", [0.5, 0.5, 0.5])
    std = config.get("data", {}).get("std", [0.5, 0.5, 0.5])
    transform = get_transform(mean, std)

    # Load models (assuming monocular forward pass):
    ft_model, base_model, ma_model = load_models(args.ft_checkpoint, args.base_checkpoint, args.ma_checkpoint, device)

    # Load evaluation dataset (monocular: each sample is a single thermal image)
    ...
    # Reference scale factor if GT is not available
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(test_loader, desc="Evaluating")):
            if batch is None:
                continue
            # For monocular evaluation, we only have one input image.
            input_img = batch["thermal"].to(device)  # [B, 3, H, W]
            # Optional: if you want to use RGB as reference:
            ...
            ft_pm1 = ft_outputs["pointmap1"]  # [B, 3, H, W]
            ft_pred_depth = ft_pm1[:, 2:3, :, :]

            # --- Base DUSt3R prediction ---
            base_outputs = base_model(input_img, input_img)
            base_pm1 = base_outputs["pointmap1"]  # [B, 3, H, W]
            base_pred_depth = base_pm1[:, 2:3, :, :]

            # --- MASt3R prediction ---
            # For MASt3R, if it expects stereo input but you're doing monocular, you may duplicate input_img.
            from dust3r.inference import inference
            dummy_instance = torch.zeros((1,1,input_img.shape[-2], input_img.shape[-1]), device=device)
            # Use the same image for both views (since it's monocular)
            image_pair = [(
            ...
            if "gt_depth" in batch:
                ...
                if valid_mask_ft.sum() > 100:
                    scale_ft = (gt_depth[valid_mask_ft] * ft_pred_depth[valid_mask_ft]).sum() / ((ft_pred_depth[valid_mask_ft] ** 2).sum() + 1e-8)
                else:
                    scale_ft = ref_scale

                valid_mask_comb = (gt_depth > 0) & (combined_pred_depth > 0) & torch.isfinite(gt_depth) & torch.isfinite(combined_pred_depth)
                if valid_mask_comb.sum() > 100:
                    scale_comb = (gt_depth[valid_mask_comb] * combined_pred_depth[valid_mask_comb]).sum() / ((combined_pred_depth[valid_mask_comb] ** 2).sum() + 1e-8)
                else:
                    scale_comb = ref_scale
            else:
                scale_ft = ref_scale
                scale_comb = ref_scale
            ...
            # Qualitative visualization (save first few samples)
            if vis_count < args.vis_samples:
                ...
                # For visualization, use our new function:
